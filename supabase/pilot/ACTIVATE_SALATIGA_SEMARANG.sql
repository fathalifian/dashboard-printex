-- Activation for the requested Salatiga/Semarang pilot. No passwords in this file.
-- Existing orders and profiles are assigned to Salatiga; Semarang starts empty.
BEGIN;
SET LOCAL printex.legacy_branch='Salatiga';
...
-- 0020_optional_customer_phone.sql
...
-- PILOT ONLY: run after migrations 0013-0020.
-- Set printex.legacy_branch to Salatiga or Semarang before this transaction.
...
DO $$ BEGIN IF (SELECT count(*) FROM public.profiles WHERE id IN ('a542dd4f-5967-4892-9ea1-fa386e947ef4','f41189f2-4bf1-43b1-9812-cfb60ae21587','b4d51953-05eb-40fd-a6df-4b7bba2bb249'))<>3 THEN RAISE EXCEPTION 'Akun uji belum tersedia pada proyek ini'; END IF; END $$;
UPDATE public.profiles SET role='central_owner',is_active=true,branch_id=NULL WHERE id='a542dd4f-5967-4892-9ea1-fa386e947ef4' AND EXISTS(SELECT 1 FROM auth.users WHERE id='a542dd4f-5967-4892-9ea1-fa386e947ef4' AND email='owner.pusat@printex.test');
UPDATE public.profiles SET role='admin',is_active=true,branch_id=(SELECT id FROM public.branches WHERE name='Salatiga') WHERE id='f41189f2-4bf1-43b1-9812-cfb60ae21587' AND EXISTS(SELECT 1 FROM auth.users WHERE id='f41189f2-4bf1-43b1-9812-cfb60ae21587' AND email='admin.salatiga@printex.test');
UPDATE public.profiles SET role='admin',is_active=true,branch_id=(SELECT id FROM public.branches WHERE name='Semarang') WHERE id='b4d51953-05eb-40fd-a6df-4b7bba2bb249' AND EXISTS(SELECT 1 FROM auth.users WHERE id='b4d51953-05eb-40fd-a6df-4b7bba2bb249' AND email='admin.semarang@printex.test');
COMMIT;
