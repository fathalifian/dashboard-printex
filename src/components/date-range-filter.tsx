    ...
    {open&&<div aria-label="Kalender rentang tanggal" className="date-filter-popover absolute right-0 top-full z-30 mt-2 w-[min(320px,calc(100vw-48px))] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
      <div className="mb-3 flex items-center justify-between gap-1">
        <div className="flex shrink-0">
          ...
          <button type="button" aria-label="Bulan sebelumnya" title="Bulan sebelumnya" onClick={()=>changeMonth(-1)} className="h-8 w-7 rounded-md text-sm font-semibold text-blue-600 hover:bg-slate-100">{'<'}</button>
        </div>
        ...
        <div className="flex shrink-0">
          <button type="button" aria-label="Bulan berikutnya" title="Bulan berikutnya" onClick={()=>changeMonth(1)} className="h-8 w-7 rounded-md text-sm font-semibold text-blue-600 hover:bg-slate-100">{'>'}</button>
          <button type="button" aria-label="Tahun berikutnya" title="Tahun berikutnya" onClick={()=>changeMonth(12)} className="h-8 w-7 rounded-md text-sm font-semibold text-blue-600 hover:bg-slate-100">{'>>'}</button>
        </div>
      </div>
      <div className="grid grid-cols-7 gap-1">{['Sen','Sel','Rab','Kam','Jum','Sab','Min'].map(day=><span key={day} className="py-1 text-center text-xs text-slate-500">{day}</span>)}
        ...
        {Array.from({length:count},(_,i)=>{
          ...
          return <button key={day} type="button" aria-label={label(day)} aria-pressed={within} onClick={()=>select(day)} onMouseEnter={()=>setHover(day)} onFocus={()=>setHover(day)} className={`h-9 rounded-lg text-sm focus-visible:outline-2 focus-visible:outline-blue-500 ${edge?'bg-blue-600 font-bold text-white':within?'bg-blue-50 font-medium text-blue-700':'text-slate-700 hover:bg-slate-100'}`}>{i+1}</button>
        })}
      </div>
      {error&&<p role="alert" className="mt-2 text-xs text-red-600">{error}</p>}
      <div className="mt-3 flex justify-between"><button type="button" onClick={()=>preset('today')} className="text-xs font-semibold text-blue-600">Hari ini</button><button type="button" onClick={()=>{setOpen(false);setFirst(null);trigger.current?.focus()}} className="text-xs text-slate-500">Batal</button></div>
    </div>}
  </div>
}
