    ...
    {!allowed?<p className="text-slate-500">Hanya Admin Utama yang dapat mengelola akun.</p>:<>
      ...
      <form onSubmit={submit} className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <fieldset disabled={busy} className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-sm text-slate-700">Password awal<input required type="password" autoComplete="new-password" minLength={12} maxLength={128} className={input} value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/><span className="text-xs text-slate-500">Minimal 12 karakter.</span></label></>}
