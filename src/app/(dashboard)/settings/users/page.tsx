    ...
    {!allowed?<p className="text-slate-500">Hanya Owner yang dapat mengelola akun.</p>:<>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="w-full text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr>{['Pengguna','Role','Status','Aksi'].map(label=><th key={label} className="p-4">{label}</th>)}</tr></thead><tbody>
          {users.map(user=><tr key={user.id} className="border-t border-slate-100 text-slate-700"><td className="p-4"><p className="font-semibold">{user.full_name}{user.id===currentId?' (Anda)':''}</p><p className="text-xs text-slate-500">{user.email}</p></td><td className="p-4">{roles[user.role]||user.role}</td><td className="p-4">{user.is_active?'Aktif':'Nonaktif'}</td><td className="p-4"><div className="flex gap-4"><button disabled={busy} onClick={()=>{setEditing(user.id);setForm({fullName:user.full_name,email:user.email,password:'',role:normalizeRole(user.role) ?? 'operator',active:user.is_active});setMessage('')}} className="text-brand-600">Edit</button><button disabled={busy||user.id===currentId} onClick={()=>setRemoving(user)} className="text-red-600 disabled:opacity-30">Hapus</button></div></td></tr>)}
