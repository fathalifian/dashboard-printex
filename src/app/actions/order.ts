...
export async function getOrders(): Promise<SupabaseOrder[]> {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select(`
      id, spk_code, production_type, meter, customer_type, order_state, order_date, due_at, notes, created_at,
      customer:customers(name, phone),
      current_step:production_steps(code, name)
    `)
  ...
  return data.map((order: any) => ({
    ...
    current_step: order.current_step ? { code: order.current_step.code, name: order.current_step.name } : { code: 'ORDER_IN', name: 'Order Masuk' },
  }))
}
